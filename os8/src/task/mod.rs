...
pub fn exit_current_and_run_next(exit_code: i32) {
    ...
    // debug!("task {} dropped", tid);
    {
        ...
        // release allocated semaphores
        (0..sem_len)
            .for_each(|i| process_inner.sem_available[i] += process_inner.sem_allocated[tid][i]);

        // empty res for new thread with the same tid
        process_inner.sem_allocated[tid] = vec![0; sem_len];
        process_inner.sem_need[tid] = vec![0; sem_len];
        process_inner.mutex_allocated[tid] = vec![false; mutex_len];
        process_inner.mutex_need[tid] = vec![false; mutex_len];
    }
